...
def process_dataset(
    ...
    for idx in tqdm(range(min(len(test_ds), limit)), desc=f"{dataset_name} samples"):
        ...
        with torch.no_grad():
            wav = wav.to(device)
            spectrogram = preprocessor(wav).to(device)
            spectrogram = spectrogram.squeeze(2)
            outputs = model(spectrogram)  # preprocess
            logits = outputs.logits  # (1, 9736)
...
def parse_args():
    ...
    p.add_argument(
        ...
        default="/workspace/logs/predictions/audioprotopnet",
    )
    p.add_argument(
        "--data-dir",
        type=str,
