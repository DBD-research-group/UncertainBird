...
def _unwrap_to_hf_convnext(m):
    """Walk .model attributes until we hit the HF ConvNextForImageClassification."""
    seen = set()
    cur = m
    while True:
        if hasattr(cur, "convnext") and hasattr(cur, "classifier"):
            return cur  # HF ConvNextForImageClassification
...
def train(cfg):
    ...
    if cfg.get("test"):
        ...
        if cfg.get("MCTest", False):
            ...
            #target = _unwrap_to_hf_convnext(best_base)

            target = resolve_hf_convnext(best_base)
            print("HOOK TARGET TYPE:", type(target))
            print("has convnext?", hasattr(target, "convnext"))
            print("has classifier?", hasattr(target, "classifier"))
            assert hasattr(target, "convnext") and hasattr(target, "classifier"), "Not at HF ConvNeXt yet!"
            _hooks = attach_convnext_eat_hooks(
            ...
            set_convnext_eat_mc_mode(target, enabled=True)
