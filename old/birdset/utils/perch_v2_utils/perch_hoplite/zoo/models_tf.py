...
import numpy as np
from birdset.utils.perch_v2_utils.perch_hoplite.taxonomy import namespace

# from birdset.utils.perch_v2_utils.perch_hoplite.taxonomy import namespace_db
from birdset.utils.perch_v2_utils.perch_hoplite.zoo import hub
from birdset.utils.perch_v2_utils.perch_hoplite.zoo import taxonomy_model_tf
from birdset.utils.perch_v2_utils.perch_hoplite.zoo import zoo_interface
...
class BirbSepModelTF1(zoo_interface.EmbeddingModel):
    ...
    def embed(self, audio_array: Any) -> zoo_interface.InferenceOutputs:
        ...
        if self.keep_raw_channel:
            sep_chunks = np.concatenate([sep_chunks, raw_chunks[np.newaxis, :]], axis=0)
...
class SeparatorModelTF(zoo_interface.EmbeddingModel):
    ...
    def embed(self, audio_array: np.ndarray) -> zoo_interface.InferenceOutputs:
        ...
        sep_audio, all_logits, all_embeddings = self.model.infer_tf(framed_audio[:1])
...
class BirdNet(zoo_interface.EmbeddingModel):
    ...
    def embed_tflite(self, audio_array: np.ndarray) -> zoo_interface.InferenceOutputs:
...
class GoogleWhaleModel(zoo_interface.EmbeddingModel):
    ...
    def embed(self, audio_array: np.ndarray) -> zoo_interface.InferenceOutputs:
        return zoo_interface.embed_from_batch_embed_fn(self.batch_embed, audio_array)
